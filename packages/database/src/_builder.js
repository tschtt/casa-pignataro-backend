...
export default {
  where(query = {}, { isNot = false } = {}) {
    const operatorFunctions = {
      ...
      $not: ({ column, value }) => {
        return this.where({ [column]: value }, { isNot: !isNot })
      },
      $and: ({ expressions = [] }) => {
        const block = expressions.map(expression => this.where(expression)).join(') AND (')
        return `(${block})`
      },
      $or: ({ expressions = [] }) => {
        const block = expressions.map(expression => this.where(expression)).join(') OR (')
        return `(${block})`
      },
      $nor: ({ expressions = [] }) => {
        const block = expressions.map(expression => this.where(expression, { isNot: true })).join(') OR (')
        return `(${block})`
      },
      $nand: ({ expressions = [] }) => {
        const block = expressions.map(expression => this.where(expression, { isNot: true })).join(') AND (')
        return `(${block})`
      }
    }
