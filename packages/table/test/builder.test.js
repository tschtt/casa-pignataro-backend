  ...
  describe('when the expression contains a $order: { by, sort? } operation', () => {
    ...
    describe('if a sorting order is set', () => {
      ...
      it('sorts by the specified order', () => {
        const resultA = builder({ $order: { by: 'active', sort: 'asc' } })
        const resultB = builder({ $order: { by: 'password', sort: 'ASC' } })
        const resultC = builder({ $order: { by: 'code', sort: 'desc' } })
        const resultD = builder({ $order: { by: 'date', sort: 'DESC' } })
        expect(resultA).to.equals('ORDER BY `active` ASC')
        expect(resultB).to.equals('ORDER BY `password` ASC')
        expect(resultC).to.equals('ORDER BY `code` DESC')
        expect(resultD).to.equals('ORDER BY `date` DESC')
      })
